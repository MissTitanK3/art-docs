...
- ❌ Should MVP launch in 1 region or multiple?
- ❌ How to handle region-specific configuration?
- ❌ Can new regions be added without code changes?
- ❌ How to enforce data isolation across regions?
- ❌ Should escalation rules vary by region?
 ...
 **Architecture:** Same codebase, deployed 3 times with different configs

```
┌─────────────────────────────────────┐
│  Single Next.js + API Codebase      │
└─────────────────────────────────────┘
              ↓
┌──────────────┬──────────────┬───────────────┐
│              │              │               │
...
**How it works:**
- Deploy same container 3 times
- Set environment variable per region: `REGION_ID=us-west-1`
- App loads config from environment
- All queries scoped to that region
- Single database with `region_id` column on all tables
...
# 2. Deploy same codebase
docker run dispatch:latest
...
|------|-------------|------------------|
| Dispatches |  View/Edit | ❌ 403 Forbidden |
| Users |  View (admin only) | ❌ 403 Forbidden |
| Audit logs |  View (admin only) | ❌ 403 Forbidden |
| Notifications |  Sent to region users | ❌ Not sent |
...
Path: nearby pod → full region → neighboring regions → all users
```

**Denver (mountain, slower response):**
```
Escalation after 15 minutes with no response
Path: geographic zone → full region → neighboring regions → all users
```
...
- Responder in us-west-1 ≠ Responder in us-east-1
