...
|--------|------------|------------------|
| View dispatches |  YES | ❌ NO (403) |
| View users |  YES | ❌ NO (403) |
| View audit logs |  YES (admin only) | ❌ NO (403) |
| Send notifications |  YES (users in region) | ❌ NO (silently skipped) |
...
GET /dispatches
→ Returns dispatches for: us-west-1 (user's only region)

# Multi-region user (must specify)
GET /dispatches?region=us-west-1
→ Returns dispatches for: us-west-1
GET /dispatches?region=us-east-1
→ Returns dispatches for: us-east-1

# Cross-region access attempt
GET /dispatches?region=us-west-2
→ 403 Forbidden (user not authorized for this region)
```
...
→ Backend assigns region from user's allowed_regions
...
→ Returns users with us-west-1 in allowed_regions
...
→ 201 Created (user can only access us-west-1)
```
...
❌ Change region IDs or structure
```
...
Test 2: Multi-region user can see both, but not others
  ...
  - GET /dispatches?region=us-west-1 →  200 (sees dispatch)
  - GET /dispatches?region=us-east-1 →  403 Forbidden
...
│  │                                  │  │
│  │  region_id from environment      │  │
│  │  config loaded on startup        │  │
│  │                                  │  │
│  │  http://dispatch.portland.org   │  │
│  │  http://dispatch.denver.org     │  │
│  │  http://dispatch.boston.org     │  │
...
│  │                                  │  │
│  │  region_id column on all tables  │  │
│  │  separate logical "regions"      │  │
...
Configure region routing via subdomains and per-region database URLs in the deployment platform:

- `dispatch.portland.org` → region `PNW`
- `dispatch.denver.org` → region `DEN`
- `dispatch.boston.org` → region `BOS`

Set the corresponding `DB_<REGION>_URL` environment variables.

### DNS Configuration

```
dispatch.portland.org  → api.dispatch.org (load balancer)
dispatch.denver.org    → api.dispatch.org (load balancer)
dispatch.boston.org    → api.dispatch.org (load balancer)

Routing uses the Host header to resolve region per request.
```
