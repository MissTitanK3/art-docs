...
2. **Implement error responses (400, 422, 500)**
   ...
   - Test: Invalid location → 422, missing body → 400, server error → 500
...
```
✓ Location Input
  ✓ Valid lat/lon (e.g., 40.7128, -74.0060) → accepted
  ✓ Invalid lat (e.g., 91.0) → rejected with error
  ✓ Invalid lon (e.g., 200.0) → rejected with error
  ✓ Missing location → cannot submit (button disabled)
  ✓ Approximate location (e.g., "Brooklyn") → accepted if implementation supports

✓ Description Input
  ✓ Empty → accepted (optional per intent)
  ✓ 500+ characters → accepted (no limit)
  ✓ Special characters → accepted (tested: é, ü, emojis if allowed)
  ✓ SQL injection attempts → safe (no raw SQL execution)

✓ Urgency Selection
  ✓ Default is "normal" (pre-selected on load)
  ✓ Each option selectable (low, normal, critical)
  ✓ Pressing each button → correct value in form state
  ✓ Cannot deselect (always one selected)

✓ Form Submission
  ✓ All fields filled → POST /dispatches succeeds, returns 201
  ✓ client_id present in request → verified in server logs
  ✓ Response contains dispatch_id → can retrieve via GET /dispatches/:id
  ✓ Submit button disabled during request (no double-submit)
  ✓ Success state visible → message appears (e.g., "Report is live")
```

#### API Integration Tests

```
✓ POST /dispatches Integration
  ✓ Form data maps correctly to API contract
  ✓ location_lat, location_lon, description, urgency in payload
  ✓ submitter_id from session token (auto-captured)
  ✓ timestamp auto-set server-side
  ✓ Idempotency: second POST with same client_id → 200 (not 201)

✓ Error Handling
  ✓ Invalid coordinates (lat > 90) → 422 with error_code="invalid_location"
  ✓ Missing location → 400 with error_code="missing_field"
  ✓ Network timeout → user sees "Try again" button, retry succeeds
  ✓ Server error (500) → user sees clear message, retry available

✓ Success Path
  ✓ Dispatch persists in database (verify via GET /dispatches)
  ✓ Dispatch record contains all fields from form
  ✓ Created timestamp is current (within 1 minute)
  ✓ Status is "open" (default)
```
...
```
✓ Language Compliance
  ✓ No forbidden terms (no "confirm", "submit", "proceed")
  ✓ Buttons are clear verbs (e.g., "Send Report")
  ✓ Error messages are actionable (not "Error" or "Invalid input")
  ✓ Help text present for each field (e.g., "A few words is enough")

✓ Accessibility
  ✓ Form has ARIA labels for all inputs
  ✓ Error messages associated with form fields (aria-describedby)
  ✓ Keyboard navigation: Tab through all fields and submit button
  ✓ Focus visible on all interactive elements
  ✓ No color-only error indication (icon or text required)
```
...
- **Submitter Capture:** How is submitter identified—automatically from session, or explicitly entered?
...
|----------|--------|-------|---------|
| API contract stable | 🟢 | Backend Lead | POST /dispatches finalized in Sprint 0 ([API Spec](/specs/backend-api-contracts)) |
| Location input method | 🟢 | Frontend Lead | Supports GPS, address, map-based input per region ([Minimal Dispatch Flow](/specs/minimal-dispatch-flow)) |
| Submitter capture auto vs. explicit | 🟢 | Backend Lead | Auto-captured from session with auth architecture ([Auth Spec](/specs/authentication-architecture)) |
| Error message format | 🟢 | Backend Lead | Standardized contract in B1 spec ([Backend API](/specs/backend-api-contracts#error-response-format)) |
| Language compliance review | 🟢 | Design Lead | All approved copy patterns available ([Language Guidelines](/language/approved-copy), [Forbidden Terms](/language/forbidden-language)) |
| Accessibility testing scope | 🟢 | QA Lead | WCAG 2.1 AA level; form validation and error states covered |
| Offline queueing impact on form | 🟢 | Product Lead | Offline support finalized; intake form assumes network available, Sprint 5 adds queuing ([Offline Spec](/specs/offline-support-architecture)) |
| Auth roles impact on intake | 🟢 | Security Lead | Three core roles defined; intake form captures submitter_id from session ([Auth Spec](/specs/authentication-architecture)) |
| Multi-region impact on form | 🟢 | Ops Lead | Three initial regions (Portland, Denver, Boston); form location defaults by region config ([Region Spec](/specs/region-scope-architecture)) |
| Crisis vs. Management mode impact | 🟢 | Product Lead | Intake form in Crisis Mode (default); Management Mode for config only ([Mode Strategy](/specs/crisis-management-mode-strategy)) |
...
| Item | Status | Blocker? |
|------|--------|----------|
| Backend API deployed to staging |  YES | ❌ NO |
| Database tables created |  YES | ❌ NO |
| API error format documented |  YES | ❌ NO |
| Design finalized for intake form |  YES | ❌ NO |
| Auth architecture finalized |  YES | ❌ NO |
| Offline support finalized |  YES | ❌ NO |
| Region scope finalized |  YES | ❌ NO |
| Crisis/Management modes finalized |  YES | ❌ NO |
| Constitution compliance reviewed |  YES | ❌ NO |
...
```
Phase A
├─ Backend: Finalize error handling and idempotency
├─ Frontend: Build location + description + urgency components
├─ Frontend: Build form container and submission logic
└─ QA: Prepare test plans and mock data

Phase B
├─ Frontend: Integrate form with real API
├─ Frontend: Language compliance review (zero forbidden terms)
├─ Backend: Deploy monitoring and alerting
├─ QA: Execute form validation tests
├─ QA: Execute API integration tests
└─ QA: Execute language compliance audit

Phase C
├─ Frontend: Accessibility audit (WCAG 2.1 AA)
├─ QA: Execute accessibility tests
├─ All: Final release checklist
└─ Release: Deploy to staging for user testing

Success Criteria Met:
├─ First-time user can submit dispatch in < 2 minutes
├─ All copy uses approved language, zero forbidden terms
├─ Dispatch records persist with all required fields
├─ Form works against real API, handles errors gracefully
└─ Ready for Sprint 2 (dispatch listing)
```
...
- [ ] Idempotency verified (duplicate client_id → 200, same dispatch_id)
...
**Current Status:** 🟢 **FULL GO** → Sprint 1 begins with all blockers resolved
